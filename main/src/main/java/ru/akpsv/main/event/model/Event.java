...
public class Event {
    ...
    private Long id; //Идентификатор. (Заполняется базой)
    @NotBlank
    ...
    private Long initiatorId; //Пользователь (краткая информация) (Заполняется eventMapper.toEvent())
    ...
    private Long confirmedRequests; //Количество одобренных заявок на участие в данном событии (Заполняется в процессе работы)
    ...
    @Builder.Default
    private EventState state = EventState.PENDING; //Список состояний жизненного цикла события Enum: [ PENDING, PUBLISHED, CANCELED ] (Заполняется во время создания)
    ...
    @Column(name = "views")
    @Builder.Default
    private Long views = 0L; //Количество просмотрев события (Заполняется во время работы)
}
